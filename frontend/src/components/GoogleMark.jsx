...
/**
 * The Google "G", drawn in `currentColor`.
 *
 * The official mark is four-colour. This site's design contract allows exactly
 * one accent colour on a monochrome canvas (see design.md), so the glyph is
 * rendered as a single solid mark and inherits the button's ink instead. The
 * silhouette is unchanged, so it still reads instantly as Google.
 */
export default function GoogleMark({ size = 15 }) {
  return (
    <svg
      width={size}
      height={size}
